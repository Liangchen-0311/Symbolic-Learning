#!/usr/bin/env python3
"""CIFAR-100: extract features from bank -> standardize on GPU -> train MLP."""
...
def main():
    ...
    all_results = {}
    configs = [
        ("h1024_baseline",            1024, 0.3, 0.0, 0.0, 1e-4),
        ("h1024_ls0.1",               1024, 0.3, 0.1, 0.0, 1e-4),
        ("h1024_mix0.2",              1024, 0.3, 0.0, 0.2, 1e-4),
        ("h1024_ls0.1_mix0.2",        1024, 0.3, 0.1, 0.2, 1e-4),
        ("h1024_ls0.1_mix0.2_wd5e-4", 1024, 0.3, 0.1, 0.2, 5e-4),
        ("h2048_ls0.1_mix0.2",        2048, 0.3, 0.1, 0.2, 1e-4),
        ("h2048_ls0.1_mix0.2_wd5e-4", 2048, 0.3, 0.1, 0.2, 5e-4),
    ]

    for label, hidden, dropout, ls, mixup, wd in configs:
        print(f"\n{'='*60}")
        print(f"  {label}")
        print(f"{'='*60}")
        result = train_mlp(
        ...
        with open(os.path.join(output_dir, 'eval_mlp_results.json'), 'w') as f:
            json.dump(all_results, f, indent=2)

    # Report
    print(f"\n{'='*60}")
    print(f"  FINAL RESULTS - {dataset_name.upper()} ({input_dim} dims)")
    print(f"{'='*60}")
    print(f"  {'Config':<35} {'Test':>7}  {'Train':>7}  {'Epoch':>5}")
    print(f"  {'-'*35} {'-'*7}  {'-'*7}  {'-'*5}")
    for name, r in sorted(all_results.items(), key=lambda x: -x[1]['test_accuracy']):
        print(f"  {name:<35} {r['test_accuracy']*100:>6.2f}%  "
              f"{r['train_accuracy']*100:>6.2f}%  {r['best_epoch']:>5}")
    print(f"{'='*60}\n")
    print(f"Results saved to {output_dir}/eval_mlp_results.json")
